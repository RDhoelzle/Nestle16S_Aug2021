...
env.all <- read.csv2('../Data/env.csv', header = T, sep = ',', row.names = 1)

# Get OTU table

otu.tmp <- read.table('../Data/otu_with_tax_4700.csv', header=TRUE, sep=',', row.names=1)
otu.all <- as.data.frame(t(otu.tmp[,-80]/4700)) # transpose and divide by 4700 to get relative abundances

# Get Taxonomy

taxonomy <- otu.tmp[,80] # Make a list of OTUs with the respective taxonomy
